...
<body>
    ...
    <div class="p-4 sm:ml-64 bg-gray-100 dark:bg-gray-900 min-h-screen">
        <div class="p-4 border-2 border-gray-200 dark:border-gray-700 rounded-lg mt-14 bg-white dark:bg-gray-800 shadow-lg">
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Daftar Vendor</h2>
            <!-- Search Bar -->
            <div class="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 pb-4">
                ...
                <div class="flex space-x-3 mb-4">
                    <a href="{{ route('vendor.create') }}" class="flex items-center px-3 py-2 text-white bg-blue-700 hover:bg-blue-800 rounded-lg">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                        </svg>
                        Tambah Vendor
                    </a>
                    <a href="{{ route('vendor.showimport') }}" class="flex items-center px-3 py-2 text-white bg-green-700 hover:bg-green-800 rounded-lg">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-2">
            ...
            <div class="relative overflow-x-auto sm:rounded-lg" style="max-height: 621px; overflow-y: auto;">
                <table id="vendorTable" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400 sticky top-0">
                        <tr>
                            <!-- Sortable Columns -->
                            <th class="px-6 py-3 sticky top-0 cursor-pointer" onclick="sortTable(0)">Kode Vendor
                                <span id="sortIconKode" class="inline-block"></span>
                            </th>
                            <th class="px-6 py-3 sticky top-0 cursor-pointer" onclick="sortTable(1)">Nama Vendor
                                <span id="sortIconVendor" class="inline-block"></span>
                            </th>
                            <th class="px-6 py-3 sticky top-0 cursor-pointer" onclick="sortTable(2)">Astra / Non Astra
                                <span id="sortIconAstra" class="inline-block"></span>
                            </th>
                    ...
                    <tbody>
                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="flex px-6 py-4">
                                ...
                                <a href="{{ route('vendor.edit', $vendor->id) }}" class="text-blue-600 hover:text-blue-800 ml-4">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                    </svg>
                                </a>
                                <button data-modal-target="terminateModal-{{ $vendor->id }}" data-modal-toggle="terminateModal-{{ $vendor->id }}" class="text-red-600 hover:text-red-800 ml-4">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        ...
                        <div id="terminateModal-{{ $vendor->id }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="terminateModal-{{ $vendor->id }}">
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Apakah anda yakin akan menghapus vendor ini?</h3>
                                        <form method="POST" action="{{ route('vendor.delete', $vendor->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button data-modal-hide="terminateModal-{{ $vendor->id }}" type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                                Ya
                                            </button>
                                            <button data-modal-hide="terminateModal-{{ $vendor->id }}" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                                                Tidak
                                            </button>
                                        </form>
                                    </div>
    ...
    <script>
        ...
        function sortTable(columnIndex) {
            ...
            const iconVendor = document.getElementById('sortIconVendor');
            const iconAstra = document.getElementById('sortIconAstra');
            ...
            if (columnIndex === 0) {
                ...
                iconVendor.textContent = '';
            } else if (columnIndex === 1) {
                iconVendor.textContent = isAscending ? '▲' : '▼';
                iconKode.textContent = '';
            } else if (columnIndex === 2) {
                iconAstra.textContent = isAscending ? '▲' : '▼';
                iconAstra.textContent = '';
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const rows = document.querySelectorAll('#vendorTable tbody tr'); // Select all rows from the table
